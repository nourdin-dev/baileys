...
const makeMessagesRecvSocket = (config) => {
    ...
    const getCallInfo = (callId) => {
    const call = callOfferCache.get(callId);
    if (!call) {
        return null;
    }
    ...
    */
    ...
    // تحديث معلومات المكالمة بدلاً من استبدالها
    const existingCall = callOfferCache.get(callId) || {};
    ...
    if (status === 'offer') {
        ...
        // استخراج المشاركين للمكالمات الجماعية
        if (call.isGroup) {
            const participants = (0, WABinary_1.getBinaryNodeChildren)(infoChild, 'participant')
                .map(p => p.attrs.jid)
                .filter(Boolean);
