...
const makeChatsSocket = (config) => {
  ...
  const reportMessage = async (messageKey, reason) => {
    const node = {
        tag: 'iq',
        attrs: {
            to: WABinary_1.S_WHATSAPP_NET,
        ...
        content: [
            {
                ...
                content: [
                        ...
                        content: [
                            {
                                tag: 'key',
                                attrs: {
                                    remote_jid: messageKey.remoteJid,
                                    from_me: messageKey.fromMe ? 'true' : 'false',
                                    id: messageKey.id,
                                }
                            }
                        ]
                    },
                    {
                        tag: 'reason',
                        attrs: {},
                        content: Buffer.from(reason, 'utf-8')
                    }
                ]
            }
        ]
    ...
    try {
        ...
        return {
            ...
            code: 200,
            response: response,
            details: `The message has been reported ${messageKey.id} Successfully`
        };
    } catch (error) {
        const statusCode = error.output?.statusCode || 500;
        const errorMessage = error.output?.payload?.message || 'Failed to report';
        
        return {
            status: 'error',
            code: statusCode,
            response: error,
            details: `Failed to report message ${messageKey.id}: ${errorMessage}`
        };
    }
};

const reportChat = async (jid, reason, messages = []) => {
    const node = {
        tag: 'iq',
        attrs: {
            to: WABinary_1.S_WHATSAPP_NET,
    ...
    try {
        ...
        return {
            ...
            code: 200,
            response: response,
            details: `The chat has been reported ${jid} Successfully`
        };
    } catch (error) {
        const statusCode = error.output?.statusCode || 500;
        const errorMessage = error.output?.payload?.message || 'Failed to report';
        ...
        return {
            ...
            code: statusCode,
            response: error,
            details: `Failed to report chat ${jid}: ${errorMessage}`
        };
    }
}; 
